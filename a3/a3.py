...
def make_feature_dicts(data,
    ...
    for sentence in sentences:
        for i in range(0, len(sentence)):
            ...
            if ("-DOCSTART-" in word):
                continue
            feature = getFeatures(word, token, caps, pos, chunk)
            
            if context:
                if i > 0 :
                    tempFeature = getFeatures(sentence[i-1], token, caps, pos, chunk)
...
def getFeatures(word, token=True,
                       caps=True,
                       pos=True,
                       chunk=True):
    feature = {}
...
def evaluate_combinations(train_data, test_data):
    ...
    """
    ###TODO
    inputs = product([False, True], repeat=4)
